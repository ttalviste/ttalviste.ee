...
param(
  ...
  If (Test-Path $kreFolder) {
    if($force)
    {
      ...
      WriteLine "$kreFullName already downloaded."
      return;
    }
  }
...
param(
  ...
  try {
    ...
    If (Test-Path $kreZip){
        WriteLine "Renamed pkg to $kreZip"
    }
 
    [System.Reflection.Assembly]::LoadWithPartialName("System.IO.Compression.FileSystem") | Out-Null
    [System.IO.Compression.ZipFile]::ExtractToDirectory($kreZip, $kreFolder)
 
    WriteLine "Unpacking successful"
  }
  catch [Exception] {
    WriteLine $_.Exception.Message
  }
  finally {
